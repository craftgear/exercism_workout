...
// コームソート改良版
public class CombFSort extends Applet implements Runnable, MouseListener
{
    ...
    while ( (gap = (gap * 10) / 14) > 1 ){
      ...
      if( (++j & 0x01) == 1 ){
        ...
        for (int i = size - 1; i - gap >= 0 ; i--) {
          ...
          delete_search_line( p1 );
          delete_search_line( p2 );
          draw_search_line( i - gap, Color.red );
